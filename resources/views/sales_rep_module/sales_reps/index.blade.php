  ...
  <script>
    $(document).ready(function() {
        ...
        const table = $('#salesRepsTable').DataTable({
            ...
            columns: [
                { data: 'user_id' },
                { data: 'user.user_name', render: (data) => data || '—' },
                { data: 'user.email', render: (data) => data || '—' },
                { data: 'vehicle.vehicle_number', render: (data) => data || '—' },
                { data: 'location.full_name', render: (data) => data ? data : '<span class="text-muted">No Location</span>' },
                {
                    data: 'routes',
        ...
        $('#salesRepsTable').on('click', '.editBtn', function() {
            ...
            $.get(`/api/sales-reps/${id}`, function(res) {
                if (res.status && res.data) {
                    ...
                    $('#user_id').val(d.user_id).prop('disabled', false);
                    $('#vehicle_id').val(d.vehicle_id).prop('disabled', false);
                    $('#route_id').val(d.route_id);
                    $('#assigned_date').val(d.assigned_date?.split('T')[0] || '');
                    $('#end_date').val(d.end_date?.split('T')[0] || '');
                    $('#status').val(d.status);
        ...
        $('#salesRepsTable').on('click', '.addRouteBtn', function() {
            ...
            $('#vehicle_id').val(vehicleId).prop('disabled', true);

            $('#modalTitle').text('Assign Another Route');
            $('#saveBtn').text('Assign Route');
            $('.text-danger').text('');

            // Load available routes excluding already assigned
            $.when(
                $.ajax({ url: '/api/sales-reps', method: 'GET' }),
                $.ajax({ url: '/api/routes', method: 'GET' })
            ).then((salesRes, routeRes) => {
                const assignedRouteIds = salesRes[0].data
                    .filter(g => g.user_id == userId && g.vehicle_id == vehicleId)
                    .flatMap(g => g.routes.map(r => r.route_id));
                ...
                if (routeRes[0].status && Array.isArray(routeRes[0].data)) {
                    routeRes[0].data
                        ...
                        .forEach(r => {
                            $('#route_id').append(`<option value="${r.id}">${r.name}</option>`);
                        });
                }
            });
