        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            async function fetchSalesData(filters) {
                try {
                    ...
                    // Simulate API call using static data for now
                    const apiData = {!! json_encode($apiData ?? []) !!}; // Replace with actual API URL if available
                    populateCustomerDropdown(apiData.sales || []);
                    populateTable(apiData.sales || [], apiData.salesReturns || []);
                    updateSummaries(apiData.summaries || {});
                } catch (error) {
                    console.error('Error fetching sales data:', error);
                    populateCustomerDropdown([]);
                    populateTable([], []);
            ...
            function populateTable(sales, salesReturns) {
                ...
                // Column visibility dropdown logic
                function updateDropdownHighlights() {
                    $('#columnVisibilityDropdown a').each(function() {
                        const value = $(this).data('value');
                        if (value === "hide all") {
