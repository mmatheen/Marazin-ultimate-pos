...
<body>
    <div class="invoice-page">
        ...
        <table class="items-table">
                ...
                @forelse ($displayItems as $item)
                    ...
                    @if ($productVatTotal > 0)
                        <tr style="background-color: #f9f9f9;">
                            <td colspan="8" style="text-align: right; font-size: 10px; color: #666;">
            ...
            @php
                // Subtotal = sum of line totals at selling price (item/MRP savings already included).
                // Only invoice-level discount is applied on top of subtotal — do not show MRP savings again here.
                $bill_discount = 0;
            ...
            <div class="summary-section">
                {{-- LEFT: Balance B/F only (not above Bill Total) --}}
                @if (!in_array($sale->status, ['quotation', 'draft']) && (!isset($sale->transaction_type) || $sale->transaction_type !== 'sale_order') && isset($customer_outstanding_balance) && $customer_outstanding_balance > 0 && round($previous_outstanding) > 0)
                    <div class="summary-column summary-column-left">
                        <div class="summary-row summary-row-left">
                            <span>Balance B/F:</span>
                            <span>{{ number_format($previous_outstanding, 2) }}</span>
                        </div>
                    </div>
                @endif
                {{-- RIGHT: discount, Bill Total, balance due --}}
