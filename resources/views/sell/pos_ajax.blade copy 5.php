...
<script>
                ...
                .then(data => {
                    ...
                    if (data.status === 200 && Array.isArray(data.data)) {
                        ...
                        allProducts = stockData.map(stock => ({
                            ...
                            location_id: stock.batches.length > 0 && stock.batches[0]
                                .location_batches.length > 0 ?
    ...
    // Check if product already exists in the billing table
    const billingBody = document.getElementById('billing-body');
    ...
    // If product exists, just increase quantity by 1 if stock allows
    if (existingRows.length > 0) {
        ...
        if (currentQuantity < maxQuantity || product.stock_alert === 0) {
            ...
            toastr.error(`You cannot add more than ${maxQuantity} units of this product.`, 'Warning');
        }
        return;
    }
    ...
    if (!Array.isArray(stockEntry.batches) || stockEntry.batches.length === 0) {
        if (product.stock_alert === 0) {
            ...
            addProductToBillingBody(product, stockEntry, product.retail_price, "all", Infinity, 'retail', 1);
        } else {
            // Product doesn't have unlimited stock but has no batches - treat as available
            locationId = product.location_id || 1;
            addProductToBillingBody(product, stockEntry, product.retail_price, "all", totalQuantity, 'retail', 1);
        }
        return;
    }
...
function updateRowSubtotal(row) {
    ...
    `;
        ...
        function addProductToBillingBody(product, stockEntry, price, batchId, batchQuantity, priceType,
            ...
            // Find existing row - check both product ID and batch ID to prevent duplicates
        ...
        function attachRowEventListeners(row, product, stockEntry) {
            ...
            function validateAndUpdateQuantity(newQuantity) {
                const maxQuantity = parseInt(priceInput.getAttribute('data-quantity'), 10);
                const productId = parseInt(row.querySelector('.product-id').textContent.trim(), 10);

                // Find the product in stockData
                const stockEntry = stockData.find(stock => stock.product.id === productId);
                const availableStock = stockEntry ? stockEntry.total_stock : 0;
                const isUnlimited = stockEntry?.product.stock_alert === 0;

                // Ensure quantity is within bounds
                if (isNaN(newQuantity) || newQuantity < 1) {
                    quantityInput.value = 1; // Reset to minimum allowed quantity
                    toastr.warning('Quantity cannot be less than 1.', 'Warning');
                    return false;
                }

                if (!isUnlimited && newQuantity > availableStock) {
                    quantityInput.value = availableStock; // Reset to maximum available quantity
                    document.getElementsByClassName('errorSound')[0]?.play();
                    toastr.error(`You cannot add more than ${availableStock} units of this product.`, 'Error');
                    return false;
                }

                // Update the input and totals
                quantityInput.value = newQuantity;
                updateTotals();
                return true;
            }
