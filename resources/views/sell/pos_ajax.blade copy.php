...
<script>
    ...
    `;
        ...
        function addProductToTableWithDetails(product, totalQuantity, locationId, locationBatches, stockEntry) {
            ...
            if (existingRow) {
                ...
                row.innerHTML = `
                    <td>
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <div class="row">
                                    <div class="col-4">
                                         <div class="font-weight-bold product-name"><a href="#" data-toggle="modal" data-target="#productModal">${product.product_name}</a></div>
                    ...
                    <td><input type="number" value="${finalPrice.toFixed(2)}" class="form-control price-input separater_amount" data-currency></td>
                ...
                row.querySelector('.product-name a').addEventListener('click', (event) => {
                event.preventDefault();
                selectedProduct = product;
                selectedRow = row;
                showModalWithDetails(product, totalQuantity, locationId, stockEntry.batches);
            });
        ...
        function generateBatchOptions(batches) {
        return batches.flatMap(batch => batch.location_batches.map(locationBatch => ({
            batch_id: batch.id,
            batch_no: batch.batch_no,
            retail_price: parseFloat(batch.retail_price),
            wholesale_price: parseFloat(batch.wholesale_price),
    ...
    function showModalWithDetails(product, totalQuantity, locationId, batches) {
        ...
        modalBody.innerHTML = `
            ...
            <div class="form-group mt-3">
                <label for="batchDropdownModal">Select Batch</label>
                <select class="form-select batch-dropdown-modal" id="batchDropdownModal" aria-label="Select Batch" style="border: 1px solid #ced4da;">
                    <option value="all" data-retail-price="${product.retail_price}" data-quantity="${totalQuantity}">
                        All - Qty: ${totalQuantity} - Price: ${product.retail_price.toFixed(2)}
                    </option>
                    ${generateBatchOptions(batches)}
                </select>
            </div>
            <div class="form-group mt-3">
                <label>Select Price Type</label>
                <div class="radio-group-modal">
                    <label><input type="radio" name="price-type-modal" value="retail" checked> <i class="fas fa-shopping-cart"></i> Retail</label>
                    <label class="ms-2"><input type="radio" name="price-type-modal" value="wholesale"> <i class="fas fa-box"></i> Wholesale</label>
                    <label class="ms-2"><input type="radio" name="price-type-modal" value="special"> <i class="fas fa-star"></i> Special</label>
                </div>
            </div>
            <div class="form-group mt-3">
                <label for="quantityModal">Quantity</label>
                <input type="number" id="quantityModal" value="1" min="1" max="${totalQuantity}" class="form-control">
            </div>
        `;

        document.getElementById('saveChangesBtn').onclick = () => {
            const selectedOption = document.querySelector('.batch-dropdown-modal').selectedOptions[0];
            const selectedPriceType = document.querySelector('input[name="price-type-modal"]:checked').value;
            const quantity = parseInt(document.getElementById('quantityModal').value, 10);
            const price = parseFloat(selectedOption.getAttribute(`data-${selectedPriceType}-price`));

            if (selectedRow) {
                const quantityInput = selectedRow.querySelector('.quantity-input');
                const priceInput = selectedRow.querySelector('.price-input');
                const batchDropdown = selectedRow.querySelector('.batch-dropdown');
                const radioGroup = selectedRow.querySelectorAll(`input[name="price-type-${product.id}"]`);

                quantityInput.value = quantity;
                priceInput.value = price.toFixed(2);
                batchDropdown.innerHTML = generateBatchOptions(batches);
                batchDropdown.value = selectedOption.value;

                radioGroup.forEach(radio => {
                    if (radio.value === selectedPriceType) {
                        radio.checked = true;
                    }
                });

                if (selectedPriceType === 'retail') {
                    selectedRow.querySelector('.product-name a').innerHTML = `${selectedProduct.product_name} <span class="star">!</span>`;
                } else if (selectedPriceType === 'wholesale') {
                    selectedRow.querySelector('.product-name a').innerHTML = `${selectedProduct.product_name} <span class="star">!!</span>`;
                } else if (selectedPriceType === 'special') {
                    selectedRow.querySelector('.product-name a').innerHTML = `${selectedProduct.product_name} <span class="star">!!!</span>`;
                }
