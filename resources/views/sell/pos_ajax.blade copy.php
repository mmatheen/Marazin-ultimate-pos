...
//     fetch('/main-category-get-all') // Adjust API endpoint if needed
...
//         cardTitle.textContent = category.categoryName;
//         card.appendChild(cardTitle);

//         card.addEventListener('click', () => {
//             handleCategoryClick(category.id); // Fetch and display subcategories
//         });
...
//     fetch(`/subcategory-get-by-category/${categoryId}`) // Adjust API endpoint if needed
//         .then(response => response.json())
//         .then(data => {
...
//     subcategoryContainer.innerHTML = ''; // Clear current subcategories
...
//         card.addEventListener('click', () => {
//             filterProductsByCategory(subcategory.id); // Display products for this subcategory
//         });
...
//                                 <img src="assets/images/${product.product_image}" alt="${product.product_name}" class="p-2 card-img-top">
...
//         globalStockData = stockResponse.message || {}; // Store stock data globally
//         globalLocationData = stockResponse.message.reduce((acc, stock) => {
//             acc[stock.product_id] = stock.location || 'Unknown'; // Map locations by product_id
//             return acc;
//         }, {});
...
//         stockResponse.message.forEach(stock => {
...
//                 };
//             }
//             stockMap[productId].totalQuantity += parseInt(stock.quantity, 10); // Accumulate quantities
//         });

//         purchaseResponse.purchases.forEach(purchase => {
...
//                     };
//                 }
//                 purchaseMap[productId].totalQuantity += parseInt(purchaseProduct.quantity, 10); // Accumulate quantities
//             });
//         });

//         // Now you can access stock data globally
//         products.forEach(product => {
//             const stockEntry = stockMap[product.id] || null;
//             const purchaseEntry = purchaseMap[product.id] || null;

//             const quantity = stockEntry ? stockEntry.totalQuantity : (purchaseEntry ? purchaseEntry.totalQuantity : 0);
//             const price = purchaseEntry ? purchaseEntry.price : (stockEntry ? stockEntry.retail_price : 0);

//             let locationName = globalLocationData[product.id] || 'Unknown'; // Use global location data

//             const cardHTML = createProductCardHTML(product, quantity, price, locationName);
//             productContainer.insertAdjacentHTML('beforeend', cardHTML);
//         });
//     })
//     .catch(error => {
...
//     // Fetch categories and populate the offcanvas
...
//         const cardTitle = document.createElement('h6');
//         cardTitle.textContent = category.mainCategoryName || category.categoryName;
//         card.appendChild(cardTitle);
...
//         nextButton.textContent = 'Next';
//         nextButton.classList.add('btn');
//         nextButton.addEventListener('click', () => fetchSubcategories(category.id));
...
//         card.addEventListener('click', () => filterProductsByCategory(subcategory.id));
...
//                 if (products.length) {
//                     products.forEach(product => {
//                         const productCard = createProductCardHTML(product, quantity, price, locationName);
...
//     }
    ...
    //     const currentStock = product.stock || 'N/A'; // Replace with actual stock value if available
    //     const locationName = product.location || 'Unknown'; // Replace with actual location if available
    ...
    // }
//     function createProductCardHTML(product, quantity, price, locationName) {
//     return `
//         <div class="col-3">
//             <div class="product-card">
//                 <img src="assets/images/${product.product_image}" alt="${product.product_name}" class="p-2 card-img-top">
...
//         const cardTitle = document.createElement('h6');
//         cardTitle.textContent = brand.name;
//         card.appendChild(cardTitle);
...
function fetchData() {
    ...
    .then(([productsResponse, stockResponse, purchaseResponse]) => {
        ...
        stockResponse.message.forEach(stock => {
            ...
            stockMap[productId].totalQuantity += parseInt(stock.quantity, 10);
        });
        ...
        purchaseResponse.purchases.forEach(purchase => {
            purchase.purchase_products.forEach(purchaseProduct => {
                ...
                purchaseMap[productId].totalQuantity += parseInt(purchaseProduct.quantity, 10);
            });
        });
        ...
        products.forEach(product => {
            const stockEntry = stockMap[product.id] || {};
            const purchaseEntry = purchaseMap[product.id] || {};

            const quantity = stockEntry.totalQuantity || purchaseEntry.totalQuantity || 0;
            const price = purchaseEntry.price || stockEntry.retail_price || 0;              let locationName = 'N/A'; // Default location if none is found
            let locationId = null; // Default location ID if none is found
            if (stockEntry && stockEntry.location) {
                locationName = stockEntry.location.name; // Use stock location
                locationId = stockEntry.location.id; // Get the location ID from stock data
...
function fetchCategories() {
    fetch('/main-category-get-all')
        ...
        .then(data => {
            const categories = data.message || [];
            categoryContainer.innerHTML = ''; // Clear the category container
            categories.forEach(category => {
                const card = createCategoryCard(category);
                categoryContainer.appendChild(card);
            });
        ...
        .then(data => {
            ...
            if (stockEntry && stockEntry.location) {
                ...
                locationId = product.locations[0].id; // Get the location ID from product locations
                    ...
                    const card = createProductCardHTML(product, quantity, price, locationName);
