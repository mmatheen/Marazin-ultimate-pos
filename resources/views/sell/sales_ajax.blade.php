<script>
    ...
    var purchaseValidationOptions = {
        ...
        messages: {
            ...
            delivered_to: {
                required: "Delivered To is required",
            },
            delivery_person: {
                required: "Delivery Person is required",
            },
            payment_method: {
    ...
    $(document).on('change keyup', '.purchase-quantity, .discount-percent, .product-price, #discount-amount, #discount-type, #tax-type', function() {
        updateCalculations();
    });
        ...
        .then(data => {
            ...
            if (data.status === 200 && Array.isArray(data.stocks)) {
                allProducts = data.stocks.map(stock => {
                    const product = stock.product; // Corrected from stock.products to stock.product
    ...
    function addProductToTable(product) {
        const batches = product.locations.flatMap(location => location.batches);
        ...
        const newRow = `
            <tr data-id="${product.id}">
                ...
                <td>
                    <input type="number" class="form-control product-price" value="${finalPrice}" min="0">
                </td>
                <td>
        ...
        $newRow.find('.purchase-quantity, .discount-percent, .product-price, .batch-dropdown').on('input change', function() {
            updateRow($newRow);
            updateFooter();
        });
        ...
        $newRow.find('.delete-product').on('click', function() {
            ...
            if (confirmed) {
                // Add the product back to the allProducts array
                allProducts.push(product);
                // Reinitialize the autocomplete
                initAutocomplete();

                $('#addSaleProduct').DataTable().row($newRow).remove().draw();
                toastr.success('Product removed from the table!', 'Success');
                updateFooter();
            }
        });
    }
    ...
    $('#addSalesForm').on('submit', function(event) {
        ...
        $('#addSaleProduct tbody tr').each(function(index) {
            ...
            formData.append(`products[${index}][quantity]`, parseFloat($(this).find('.purchase-quantity').val()) || 0);
            formData.append(`products[${index}][unit_price]`, parseFloat($(this).find('.product-price').val()) || 0);
            formData.append(`products[${index}][discount]`, parseFloat($(this).find('.discount-percent').val()) || 0);
            formData.append(`products[${index}][batch_id]`, $(this).find('.batch-dropdown').val());
            formData.append(`products[${index}][tax]`, parseFloat($(this).find('.product-tax').val()) || 0);
        });
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.message === 'Sale added successfully!') {
