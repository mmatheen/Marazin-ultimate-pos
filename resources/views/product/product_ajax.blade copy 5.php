...
$(document).ready(function () {
    ...
    let subCategories = [];
    let categoryMap = {};
    let subCategoryMap = {};  // Define subCategoryMap
    let brandMap = {};
    let locationMap = {};
    let unitMap = {};
    ...
    function populateProductFilter() {
        ...
        const subCategories = [...new Set(allProducts.map(item => item.product.sub_category_id))];
        ...
        const categoryMap = allProducts.reduce((acc, item) => {
            acc[item.product.main_category_id] = item.product.main_category_name;
            return acc;
        }, {});

        const subCategoryMap = allProducts.reduce((acc, item) => {
            acc[item.product.sub_category_id] = item.product.sub_category_name;
            return acc;
        }, {});

        const brandMap = allProducts.reduce((acc, item) => {
            acc[item.product.brand_id] = item.product.brand_name;
            return acc;
        }, {});

        const unitMap = allProducts.reduce((acc, item) => {
            acc[item.product.unit_id] = item.product.unit_name;
            return acc;
        }, {});

        const locationMap = allProducts.reduce((acc, item) => {
            acc[item.product.location_id] = item.product.location_name;
            return acc;
        }, {});

        populateDropdown(productNameFilter, productNames, {});
        populateDropdown(categoryFilter, categories, categoryMap);
        populateDropdown(subCategoryFilter, subCategories, subCategoryMap);
                ...
                <td>
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="actionsDropdown">
                            <li><a class="dropdown-item view_btn" href="#" data-bs-toggle="modal" data-bs-target="#viewProductModal" data-product-id="${product.id}"><i class="fas fa-eye"></i> View</a></li>
                            <li><a class="dropdown-item edit_btn" href="/edit-product/${product.id}"><i class="fas fa-edit"></i> Edit Product</a></li>
                            <li><a class="dropdown-item delete_btn" href="#" data-id="${product.id}"><i class="fas fa-trash-alt"></i> Delete</a></li>
    ...
    function filterProducts() {
        ...
        const filteredProducts = allProducts.filter(item => {
            ...
            const matchesSubCategory = selectedSubCategory ? product.sub_category_id == selectedSubCategory : true;
            const matchesBrand = selectedBrand ? product.brand_id == selectedBrand : true;
            const matchesUnit = selectedUnit ? product.unit_id == selectedUnit : true;
            const matchesLocation = selectedLocation ? product.location_id == selectedLocation : true;
            return matchesProduct && matchesCategory && matchesSubCategory && matchesBrand && matchesUnit && matchesLocation;
        });
    ...
    $('#productNameFilter, #categoryFilter, #subCategoryFilter, #brandFilter, #unitFilter, #locationFilter').on('change', filterProducts);
