<?php
/**
 * Simple Ledger Fix - Direct Database Operations
 * Works without complex validation checks
 */

require_once 'simple_database_manager.php';

echo "=== SIMPLE LEDGER FIX ===\n\n";
...
try {
    ...
    try {
        ...
        foreach ($customers as $customer) {
            ...
            if (abs($expectedBalance - $ledgerBalance) > 0.01) {
                ...
                if ($isDryRun) {
                    echo "🔍 Would fix {$customer['first_name']}: Difference = {$difference}\n";
                } else {
                    ...
                    echo "✅ Fixed {$customer['first_name']}: Balance = {$expectedBalance}\n";
                }
        ...
        foreach ($suppliers as $supplier) {
            ...
            if (abs($expectedBalance - $ledgerBalance) > 0.01) {
                ...
                if ($isDryRun) {
                    echo "🔍 Would fix {$supplier['first_name']}: Difference = {$difference}\n";
                } else {
                    ...
                    echo "✅ Fixed {$supplier['first_name']}: Balance = {$expectedBalance}\n";
                }
    ...
    if ($isDryRun) {
        echo "\n🧪 This was a dry run. Use 'php simple_fix.php' to apply changes.\n";
    } else {
