...
class ProductController extends Controller
{
    ...
    public function quickAdd(Request $request)
    {
        try {
            ...
            $product->tax_percent = TaxConfigurationService::defaultTaxPercent();
            $product->selling_price_tax_type = TaxConfigurationService::defaultSellingPriceTaxType();
        ...
        try {
            ...
            if (!empty($stockStatus)) {
                switch ($stockStatus) {
                    ...
                    case 'free_stock':
                        // Products with free stock > 0 in any location (or selected location)
                        $query->whereHas('batches.locationBatches', function ($q) use ($locationId, $userLocationIds) {
                            $q->where('free_qty', '>', 0);
                            if ($locationId) {
                                $q->where('location_id', $locationId);
