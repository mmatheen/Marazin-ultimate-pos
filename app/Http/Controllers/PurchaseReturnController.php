    ...
    public function update(Request $request, $purchaseReturnId)
    {
        ...
        try {
            DB::transaction(function () use ($request, $purchaseReturnId) {
                ...
                // Fetch the existing purchase return record
                $purchaseReturn = PurchaseReturn::findOrFail($purchaseReturnId);

                // Reverse the stock adjustments for the existing products in the return
                ...
                foreach ($existingProducts as $existingProduct) {
                    ...
                    if (empty($batchId)) {
                        $this->restoreStockFIFO($existingProduct->product_id, $purchaseReturn->location_id, $quantity);
                    } else {
                        $this->restoreBatchStock($batchId, $purchaseReturn->location_id, $quantity);
                    }
                }
