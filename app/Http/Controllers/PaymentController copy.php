...
class PaymentController extends Controller
{
    ...
    public function destroy(Payment $payment)
    {
        DB::transaction(function () use ($payment) {
            ...
            if ($payment->payment_type === 'purchase' || $payment->payment_type === 'purchase_return') {
                $this->restorePurchaseOrSale($payment, $oldAmount, $payment->payment_type);
    ...
    private function validateReference($request)
    {
        if ($request->payment_type === 'purchase') {
            ...
            return $purchaseReturn && $purchaseReturn->supplier_id == $request->supplier_id;
            ...
            return $saleReturn && $saleReturn->customer_id == $request->customer_id;
        }
        return false;
    }
        ...
        switch ($request->payment_type) {
            case 'purchase':
                ...
                if ($purchase) {
                    ...
                    $purchase->save(); // Save the total_paid without updating the generated column
            ...
            case 'sale':
                ...
                if ($sale) {
                    ...
                    $sale->save(); // Save the total_paid without updating the generated column
            ...
            case 'purchase_return':
                ...
                if ($purchaseReturn) {
                    ...
                    $purchaseReturn->updateTotalDue(); // Ensure updateTotalDue recalculates total_due correctly
            ...
            case 'sale_return_without_bill':
                ...
                if ($saleReturn) {
                    ...
                    $saleReturn->updateTotalDue(); // Ensure updateTotalDue recalculates total_due correctly
                    $this->updateSaleReturnPaymentStatus($saleReturn, $totalPaid);
                }
                break;

            default:
                throw new \Exception("Unknown payment type: {$request->payment_type}");
        }
    }
    ...
    {
        if ($purchaseReturn->return_total - $totalPaid <= 0) {
            $purchaseReturn->payment_status = 'Paid';
        } elseif ($totalPaid < $purchaseReturn->return_total) {
    ...
    private function updateSupplierBalance($supplierId)
    {
        ...
        if ($supplier) {
            ...
            $totalPayments = Payment::where('supplier_id', $supplierId)->whereIn('payment_type', ['purchase', 'purchase_return'])->sum('amount'); // Updated line
    ...
    private function updateCustomerBalance($customerId)
    {
        ...
        if ($customer) {
            ...
            $totalPayments = Payment::where('customer_id', $customerId)->whereIn('payment_type', ['sale', 'sale_return_with_bill', 'sale_return_without_bill'])->sum('amount'); // Updated line
    ...
    private function getTotalDue($request, $paymentId = null)
    {
        ...
        if ($request->payment_type === 'purchase') {
            ...
            $purchaseReturn = PurchaseReturn::find($request->reference_id);
            return $purchaseReturn ? $purchaseReturn->return_total - $totalPaid : 0; // Updated line
        } elseif ($request->payment_type === 'sale_return_with_bill' || $request->payment_type === 'sale_return_without_bill') {
            $saleReturn = SalesReturn::find($request->reference_id);
    ...
    public function show(Payment $payment)
    {
        if ($payment->payment_type === 'sale') {
            $payment->load('customer', 'reference.location'); // Ensure related data is loaded
        } elseif ($payment->payment_type === 'purchase') {
            $payment->load('supplier', 'reference.location'); // Ensure related data is loaded
        } else {
            $payment->load('customer', 'supplier'); // Load customer and supplier if applicable
        }
    ...
    private function restorePurchaseOrSale($payment, $oldAmount, $type)
    {
        switch ($type) {
            ...
            case 'purchase_return':
                ...
                if ($purchaseReturn) {
                    $totalPaid = $purchaseReturn->payments ? $purchaseReturn->payments->sum('amount') - $oldAmount : 0;
                    $purchaseReturn->total_paid = $totalPaid;
                    $purchaseReturn->updateTotalDue(); // Ensure updateTotalDue recalculates total_due correctly
                    $this->updatePurchaseReturnPaymentStatus($purchaseReturn, $totalPaid);
                }
            ...
            case 'sale_return_with_bill':
            case 'sale_return_without_bill':
                $saleReturn = SalesReturn::find($payment->reference_id);
                if ($saleReturn) {
                    $totalPaid = $saleReturn->payments ? $saleReturn->payments->sum('amount') - $oldAmount : 0;
                    $saleReturn->total_paid = $totalPaid;
                    $saleReturn->updateTotalDue(); // Ensure updateTotalDue recalculates total_due correctly
                    $this->updateSaleReturnPaymentStatus($saleReturn, $totalPaid);
                }
