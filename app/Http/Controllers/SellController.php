    ...
    public function store(Request $request)
    {
        try {
            ...
            $request->validate([
                ...
                'items.*.batch_id' => 'nullable|exists:batches,batch_id', // Added validation for batch_id
            ]);
            ...
            foreach ($request->items as $order) {
                ...
                // Reduce stock quantity based on the selected batch ID or FIFO method
                if ($remainingQuantity > 0) {
                    if (isset($order['batch_id']) && $order['batch_id'] !== 'all') {
                        // Reduce stock based on the selected batch ID
                        $batch = Batch::where('product_id', $order['product_id'])
                        ...
                        if ($batch) {
                            ...
                            throw new \Exception('Batch ID not found for product ID: ' . $order['product_id']);
                        }
                    } else {
                        // FIFO method
