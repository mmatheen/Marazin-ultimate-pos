    ...
    public function model(array $row)
    {
        ...
        $validator = Validator::make($row, [
            'sku' => [
                ...
                Rule::unique('products', 'sku')->ignore($row['sku'], 'sku') // Ensure SKU is unique (ignore current SKU if updating)
            ],
        ...
        if ($validator->fails()) {
            // Collect all the validation errors for this row
            $this->validationErrors[] = $validator->errors()->all();  // Store all error messages for this row

            // Log the error messages for debugging
            Log::error('Validation Errors for SKU:', $validator->errors()->toArray());

            // Return null to skip this row if validation fails
            return null;
        }

        // **Process the SKU if it's not provided**
        if (empty($row['sku'])) {
            // Generate a new SKU if not provided
